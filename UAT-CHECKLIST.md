...
- **Config-per-file rule (D-06):** one checklist row per `@ConfigEntity`-annotated class, never one
  row per key. One such row exists here: `ultisidebar.config.sidebar-yml`, covering all 6
  `@ConfigEntry` keys plus the legacy-line-migration behaviour in a single pass.
...
|---|---|---|---|---|---|
| ultisidebar.sidebar.toggle | `language: en`; sender holds `ultisidebar.toggle`; sender's sidebar currently OFF (no persisted preference, `default-enabled: false` — or an explicit prior `/sidebar off`); `sidebar.yml` at its shipped defaults; PlaceholderAPI installed | Run `/sidebar toggle` | Chat shows `Sidebar has been enabled!` (green); a scoreboard appears in the sidebar slot titled `My Server` (color-translated from the shipped Chinese default, `SideBarConfig.java:37`) showing the 12 configured lines (with the 3 blank separators), each PlaceholderAPI/`{player}`-substituted — e.g. the first line reads `Welcome, <player>` (translated from the shipped Chinese default) | pixel | |
...
| ultisidebar.sidebar.cache-cleanup-on-quit | The player's sidebar is currently ON (run `ultisidebar.sidebar.on` first) | The player quits, then immediately rejoins and runs `/sidebar off` | The rejoin's `/sidebar off` still succeeds normally (no stale-scoreboard exception or crash) — proving `SideBarService#playerScoreboards`/`#contentCache` no longer held an entry for the player's UUID after the quit, since nothing else could have removed it in between | server | |
