...
- **Shipped defaults containing Chinese-language content:** this module's `lines` config default
  is a literal, hardcoded Simplified Chinese template (server-owner-authored scoreboard content,
  not `i18n(...)` keys) — it does NOT respond to the framework's `language` setting at all. The
  `title` default is blank, and a blank title shows the language file's `sidebar_default_title`,
  which does follow `language`, like this module's other `i18n(...)` command/status messages, which DO have
...
| ultisidebar.config.sidebar.title | The sidebar's scoreboard objective title, `&`-color-coded and PlaceholderAPI-substituted (via `parsePlaceholders`, which ONLY calls `PlaceholderAPI#setPlaceholders` — there is NO `{player}`-token replacement anywhere in this module, unlike `Modules/UltiMenu`'s own `{player}` mechanism) on every refresh; validated `@Size(max=32)`. Shipped default is blank (`""`): while the value is blank, `SideBarConfig#getTitle` returns the language file's `sidebar_default_title` in the server's language (`"&6&lMy Server"` under `language: en`), resolved each time it is read; a non-blank value is shown as written | config | `config/sidebar.yml: title (default: blank — the language file's sidebar_default_title is shown)` | n/a | n/a | admin | detailed | SideBarConfig#getTitle, SideBarService#updateSidebar |
...
| ultisidebar.config.sidebar.legacy-title | On plugin enable and on every reload, a persisted `title` that is BYTE-IDENTICAL to the title every earlier version shipped (a Chinese literal, `SideBarConfig#SHIPPED_TITLE`, translated: `"&6&lMy Server"`) is blanked and the file saved, so the language file's `sidebar_default_title` is shown in the server's language; any other title, including an operator's own, is kept as written. Idempotent — a blank title matches nothing, so the file is not rewritten again | config | automatic on plugin enable/reload, for any `sidebar.yml` whose `title` is the old shipped title | n/a | n/a | internal | brief | SideBarConfig#migrateLegacyDefaults, SideBarService#init |
