...
| ultisidebar.sidebar.reload | Reload this module's configuration and refresh every online player's sidebar; gated by TWO permission checks — see this row's own Permission cell for the mechanism split. Delegates to `UltiToolsPlugin#reloadSelf()`, which this module OVERRIDES without calling `super.reloadSelf()` — the language catalogue is never re-created and `@ConditionalOnConfig` drift is never reported by this reload path, even though config values and the sidebar service itself DO refresh (`UltiKits/UltiSideBar#16`, filed, not fixed here) | command | `/sidebar reload` | ultisidebar.toggle + ultisidebar.admin | both | admin | detailed | SideBarCommand#reload |
...
| ID | Feature | Kind | How to reach | Permission | Target | Tier | Manual | Source |
|---|---|---|---|---|---|---|---|---|
| ultisidebar.sidebar.auto-enable-on-join | 10 ticks (0.5s) after a player joins, auto-enable their sidebar IF their persisted preference (or, absent one, `default-enabled`) says enabled — re-checks the player is still online before enabling, guarding against a join-then-immediate-quit race | event | join the server with an enabled persisted preference, or no preference and `default-enabled: true` (the shipped default) | n/a | n/a | player | brief | SideBarListener#onPlayerJoin, SideBarService#onPlayerJoin |
...
| ultisidebar.config.sidebar.default-enabled | The sidebar-enabled state assumed for a player with NO persisted preference row yet (their very first join, or after any external deletion of their row) — once a preference row exists, this key has no further effect for that player | config | `config/sidebar.yml: default-enabled (default: true)` | n/a | n/a | admin | brief | SideBarConfig#defaultEnabled, SideBarService#isSidebarEnabledInDatabase |
